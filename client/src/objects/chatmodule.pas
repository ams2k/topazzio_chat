...
begin
  ...
  if not FTCPClient.Connected then begin
    if Assigned(OnYourConnectionStatus) then
      OnYourConnectionStatus(msg + sLineBreak + 'Verifique as configurações no arquivo: chat_config.ini' + sLineBreak + Format('Server: %s:%d',[FServer_IP,FServer_Porta]), False);
